...
class LISA(nn.Module):
    ...
    def forward(self, x_lr:torch.Tensor, scale): 
        ...
        # 1. The true target time coordinates
        t_hr = torch.arange(L_hr, device=x_lr.device).float() / scale
        t_hr = t_hr.unsqueeze(0).repeat(B, 1)
        
        # 2. We ONLY perturb the coordinate used to select the latent code
        if self.training:
        ...
        # 3. Find the anchor index using the perturbed selection
        idx_i = torch.round(t_select).long().clamp(0, L_lr - 1)
        t_i = idx_i.float()
        
        # 4. CRITICAL FIX: Relative distance MUST be from the ACTUAL target time
